...
export const Changes_Page = ({selectedRData,setSelectedRData,caObjectIds}) => {
    ...
    useEffect(() => {
        ...
        fetch(`http://localhost:8081/Supplierportal/webapi/SupplierPortal/getSupplierDetailsByEmail?email=${username}`)
        .then(res => res.json())
        .then(data => {
                            ...
                            {row.acknowledged ? (
                                <>
                                    <img
                                        ...
                                        style={{ width: '32x', height: '27px', cursor: 'pointer' }}
    ...
    const fetchChangeActionDetails = async (objectId) => {
        try {
            const response = await fetch(`http://localhost:8081/Supplierportal/webapi/SupplierPortal/getchangeactiondetails?caid=${objectId}`);
    ...
    const renderGroupedFields = (details) => {
        ...
        const conditionalRowStyles = [
            {
                when: row => row.objectId === selectedRowId,
                style: {
    ...
    return (
        <div>
            ...
            <Tabs selectedIndex={selectedTab} onSelect={index => setSelectedTab(index)}>
                <TabList>
                    <Tab className={selectedTab === 0 ? 'selectedTabColourHandel' : 'nonselectedtabGrayOut'}>
                        <div className="ca-tab-content">
                            <img src={changeActionImage} className="tab-icon" />
                            <div className="tab-content-size">ChangeAction Detail(s)</div>
                        </div>
                    </Tab>
                    <Tab className={selectedTab === 1 ? 'selectedTabColourHandel' : 'nonselectedtabGrayOut'}>
                        <div className="ca-tab-content">
                            <img src={changeActionImage} className="tab-icon" />
                ...
                <TabPanel>
                    <CA_AffectedItems caId={caId} acknowledgeValue={acknowledgeValue} selectedRData={selectedRData} setSelectedRData={setSelectedRData} selectedName={selectedName}/>
                </TabPanel>
            </Tabs>
