...
const MainSection = ({
  ...
  const element = document.querySelector(".ag-overlay-no-rows-center");
  if (element) {
    element.innerHTML = "";

    element.innerHTML = `
  <div class="loading-container">
    <img src="${loadingIcon}" alt="Loading..." class="loading-logo" />
  </div>`;

    console.log("------------------------------------------ Loading icon set.");
  } else {
  ...
  const [currentViewData, setCurrentViewData] = useState(
    userPre.views.main_table_view[0].name
  );
  ...
  useEffect(() => {
    const fetchSupplierName = async () => {
      try {
        const response = await fetch(
          `${API_BASE_URL}/SupplierPortal/getSupplierName?username=${username}`
        );
    ...
    const fetchVisibility = async (partId, supplierName) => {
      try {
        const response = await fetch(
          `${API_BASE_URL}/SupplierPortal/getAssignedParts?suppliername=${supplierName}&partid=${partId}`
        );
        const result = await response.json();
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(
          `${API_BASE_URL}/SupplierPortal/parts?suppliername=${supplierName}`
        );
        const fetchDatas = await response.json();
        console.log("fetchDatas-------------", fetchDatas);

        const visiblePartIdsArray = Array.isArray(visiblePartIds)
          ? visiblePartIds
          : [];
        const includedObjectIds = new Set();

        const combinedData = await Promise.all(
          fetchDatas.results.map(async (data) => {
            console.log("Object****", Object.keys(data));
            const key = Object.keys(data)[0];
            const objId = key.split(":")[1].trim();
            const basicAttributes = data[key].basicAttributes.reduce(
              (acc, attr) => {
        ...
        const orderedDisplayNames = columnOrderConfig.columnOrder.reduce(
          (acc, key) => {
            if (displayNames[key]) {
  ...
  const generateCsvHeaders = (columnDisplayNames) => {
    return Object.entries(columnDisplayNames).map(([key, displayName]) => ({
      label: displayName,
      key: key,
    }));
  };
  ...
  const handleRowClicked = async (event) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/SupplierPortal/parts?partid=${objectId}`,
        {
          method: "GET",
          headers: {
