...
export const Tabs_Sec = ({ selectedRow,visibility,specVisibility,itemVisibility }) => {
  ...
  const [devAailableAttributes, setdevAailableAttributes] = useState([
    ...
    "Description",
    "State",
    "View Details", // Default attributes
  ]);
  const [devVisibleColumns, setdevVisibleColumns] = useState(
    devAailableAttributes
  );
  ...
  const [availableAttributes, setAvailableAttributes] = useState([
    ...
    "Synopsis",
    "Description",
    "View Affected Items", // Default attributes
  ]);
  ...
  const [ebomAvailableAttributes, setEBOMAvailableAttributes] = useState([
    "Name",
    "Description",
    "Owner", // Default attributes
  ]);
  const [ebomVisibleColumns, setebomVisibleColumns] = useState(
    ebomAvailableAttributes
  );
  ...
  const prepareExportData = useCallback(
    (includeAllColumns) => {
      const columnsToInclude = includeAllColumns
        ? columnsData
        : columnsData.filter((col) => devVisibleColumns.includes(col.name));
      const data = DevTableData.map((row) => {
        const processedRow = {};
        columnsToInclude.forEach((col) => {
          const key = col.name;
          processedRow[key] = Array.isArray(row[key])
            ? row[key].join(", ")
            : row[key] || "N/A";
        });
        return processedRow;
  ...
  const prepareCAExportData = useCallback(
    (includeAllColumns) => {
      const columnsToInclude = includeAllColumns
        ? columnsData
        : columnsData.filter((col) => visibleColumns.includes(col.name));
  ...
  const prepareSpecExportData = useCallback(
    (includeAllColumns) => {
      const columnsToInclude = includeAllColumns
        ? columnsData
        : columnsData.filter((col) => visibleColumns.includes(col.name));
  ...
  //EBOM
  const prepareEBOMExportData = useCallback(
    (includeAllColumns) => {
      const columnsToInclude = includeAllColumns
        ? columnsData
        : columnsData.filter((col) => ebomVisibleColumns.includes(col.name));
  ...
  const handleExportOptionChange = useCallback(
    ...
    [
      prepareCAExportData,
      prepareSpecExportData,
      prepareEBOMExportData,
      prepareExportData,
    ]
  );
    ...
    <TabList className="tab-list">
      ...
      <TabPanel>
        <Tab_Specification
          ...
          setExportDropdownVisible={setExportDropdownVisible}
      ...
      <TabPanel>
        <Tab_EBOM
          ...
          ebomAvailableAttributes={ebomAvailableAttributes}
          setEBOMAvailableAttributes={setEBOMAvailableAttributes}
          visibleColumns={visibleColumns}
          dataToExport={dataToExport}
          exportOption={exportOption}
          exportDropdownVisible={exportDropdownVisible}
          ...
          setebomVisibleColumns={setebomVisibleColumns}
          ebomVisibleColumns={ebomVisibleColumns}
          selectedRow={selectedRow}
        />
      </TabPanel>
      ...
      <TabPanel>
        <Tab_Deviation
          ...
          setDevTableData={setDevTableData}
          setdevAailableAttributes={setdevAailableAttributes}
          devAailableAttributes={devAailableAttributes}
          selectedRow={selectedRow}
        />
      </TabPanel>
